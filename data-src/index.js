...
class UIBinder {
    ...
    initShadesDB() {
        if (!confirm('This will create a minimal shades database (shades.cfg) on the device. Continue?')) return;
        let overlay = ui.waitMessage(document.getElementById('divContainer'));
        fetch(baseUrl.length > 0 ? `${baseUrl}/init/shades` : '/init/shades', {
            method: 'POST',
            headers: { 'apikey': security.apiKey }
        }).then(async (resp) => {
            overlay.remove();
            if (!resp.ok) {
                let txt = await resp.text();
                ui.serviceError({ htmlError: resp.status, desc: txt });
            } else {
                let json = {};
                try { json = await resp.json(); } catch (e) { json = { status: 'OK' }; }
                ui.infoMessage(document.getElementById('divContainer'), json.desc || 'Created shades.cfg');
                // reload controller state
                somfy.loadSomfy();
            }
        }).catch((err) => { overlay.remove(); ui.serviceError(err); });
    }
