      ...
      if( n >= 0) { // If the scan is complete but the WiFi isn't ready this can return 0.
        ...
        if(this->getStrongestAP(settings.WIFI.ssid, bssid, &channel)) {
          if(!WiFi.BSSID() || memcmp(bssid, WiFi.BSSID(), sizeof(bssid)) != 0) {
            if(!this->connected()) {
              LOGI("Connecting to AP %02X:%02X:%02X:%02X:%02X:%02X CH: %d", bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5], channel);
              this->connectWiFi(bssid, channel);
            }
            else {
              LOGD("Found stronger AP %02X:%02X:%02X:%02X:%02X:%02X CH: %d", bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5], channel);
...
void Network::setConnected(conn_types_t connType) {
  ...
  if(this->connType == conn_types_t::wifi) {
    ...
    if(this->softAPOpened) {
      LOGI("Disconnecting from SoftAP");
      WiFi.softAPdisconnect(true);
      WiFi.mode(WIFI_OFF);
    }
    this->connectAttempts++;
  ...
  if(this->connectAttempts == 1) {
    LOGI("");
    if(this->connType == conn_types_t::wifi) {
      LOGI("Successfully Connected to WiFi: IP=%s strength=%d dBm", WiFi.localIP().toString().c_str(), this->strength);
      if(settings.IP.dhcp) {
      ...
      LOGI("Successfully Connected to Ethernet: IP=%s speed=%dMbps fullDuplex=%s", ETH.localIP().toString().c_str(), ETH.linkSpeed(), ETH.fullDuplex() ? "true" : "false");
      if(settings.IP.dhcp) {
    ...
    LOGI("Reconnected after %.2f sec IP: %s mac:%s CH:%d strength:%d dBm Disconnected %d times",
         1.0 * (millis() - this->connectStart)/1000,
         this->connType == conn_types_t::wifi ? WiFi.localIP().toString().c_str() : ETH.localIP().toString().c_str(),
         this->mac.c_str(), this->channel, this->strength, this->connectAttempts - 1);
  }
  SSDP.setHTTPPort(80);
  ...
  if(MDNS.begin(settings.hostname)) {
    LOGI("MDNS Responder Started: serverId=%s", settings.serverId);
...
bool Network::connectWired() {
  ...
  if(this->connectAttempts > 0) {
    LOGW("Ethernet Connection Lost... %d Reconnecting %s", this->connectAttempts, this->mac.c_str());
  }
  else
    LOGI("Connecting to Wired Ethernet");
  this->_connecting = true;
  ...
  if(!this->ethStarted) {
    ...
    LOGD("Set hostname to: %s", ETH.getHostname());
    if(!ETH.begin(settings.Ethernet.phyAddress, settings.Ethernet.PWRPin, settings.Ethernet.MDCPin, settings.Ethernet.MDIOPin, settings.Ethernet.phyType, settings.Ethernet.CLKMode)) { 
      ...
      if(!settings.IP.dhcp) {
        if(!ETH.config(settings.IP.ip, settings.IP.gateway, settings.IP.subnet, settings.IP.dns1, settings.IP.dns2)) {
          LOGW("Unable to configure static IP address....");
...
void Network::updateHostname() {
  if(settings.hostname[0] != '\0' && this->connected()) {
    if(this->connType == conn_types_t::ethernet &&
      ...
      LOGD("Updating host name to %s...", settings.hostname);
      ETH.setHostname(settings.hostname);
...
bool Network::connectWiFi(const uint8_t *bssid, const int32_t channel) {
  ...
  if(bssid && channel > 0) {
    ...
    this->connType = conn_types_t::unset;
    if(this->connectAttempts > 0) {
      LOGW("Connection Lost... %s CH:%d (%ddb)", this->mac.c_str(), this->channel, this->strength);
    }
    else LOGI("Connecting to AP");
    delay(100);
    // There is also another method simply called hostname() but this is legacy for esp8266.
    if(settings.hostname[0] != '\0') WiFi.setHostname(settings.hostname);
    LOGD("Set hostname to: %s", WiFi.getHostname());
    WiFi.setScanMethod(WIFI_ALL_CHANNEL_SCAN);
    WiFi.setSortMethod(WIFI_CONNECT_AP_BY_SIGNAL);
    uint8_t _bssid[6];
    int32_t _channel = 0;
    if(!settings.WIFI.hidden && this->getStrongestAP(settings.WIFI.ssid, _bssid, &_channel)) {
      LOGD("Found strongest AP %02X:%02X:%02X:%02X:%02X:%02X CH:%d", _bssid[0], _bssid[1], _bssid[2], _bssid[3], _bssid[4], _bssid[5], _channel);
...
void Network::networkEvent(WiFiEvent_t event) {
  switch(event) {
    case ARDUINO_EVENT_WIFI_READY:               LOGD("(evt) WiFi interface ready"); break;
    case ARDUINO_EVENT_WIFI_SCAN_DONE:           
      LOGD("(evt) Completed scan for access points (%d)", WiFi.scanComplete()); 
      //Serial.println("(evt) Completed scan for access points");
      net.lastWifiScan = millis();
      break;
    case ARDUINO_EVENT_WIFI_STA_START:
      LOGD("WiFi station mode started");
      if(settings.hostname[0] != '\0') WiFi.setHostname(settings.hostname);
      break;
    case ARDUINO_EVENT_WIFI_STA_STOP:            LOGD("(evt) WiFi clients stopped"); break;
    case ARDUINO_EVENT_WIFI_STA_CONNECTED:       LOGD("(evt) Connected to WiFi STA access point"); break;
    case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:    
      LOGW("(evt) Disconnected from WiFi STA access point. Connecting: %d", net.connecting());
      net.connType = conn_types_t::unset;
      net.disconnectTime = millis();
      net.clearConnecting();
      break;
    case ARDUINO_EVENT_WIFI_STA_AUTHMODE_CHANGE: LOGD("(evt) Authentication mode of STA access point has changed"); break;
    case ARDUINO_EVENT_WIFI_STA_GOT_IP:
      LOGI("(evt) Got WiFi STA IP: %s", WiFi.localIP().toString().c_str());
      net.connType = conn_types_t::wifi;
      net.connectTime = millis();
      net.setConnected(conn_types_t::wifi);
      break;
    case ARDUINO_EVENT_WIFI_STA_LOST_IP:        LOGW("Lost IP address and IP address is reset to 0"); break;    
    case ARDUINO_EVENT_ETH_GOT_IP:
      // If the Wifi is connected then drop that connection
      if(WiFi.status() == WL_CONNECTED) WiFi.disconnect(true);
      LOGI("Got Ethernet IP %s", ETH.localIP().toString().c_str());
      net.connectTime = millis();
    ...
    case ARDUINO_EVENT_ETH_CONNECTED:
      LOGD("(evt) Ethernet Connected");
      break;
    case ARDUINO_EVENT_ETH_DISCONNECTED:
      LOGW("(evt) Ethernet Disconnected");
      net.connType = conn_types_t::unset;
      net.disconnectTime = millis();
      net.clearConnecting();
      break;
    case ARDUINO_EVENT_ETH_START:               
      LOGD("(evt) Ethernet Started"); 
      net.ethStarted = true;
      break;
    case ARDUINO_EVENT_ETH_STOP:
      LOGD("(evt) Ethernet Stopped");
      net.connType = conn_types_t::unset;
      net.ethStarted = false;
      break;
    case ARDUINO_EVENT_WIFI_AP_START:
      LOGD("(evt) WiFi SoftAP Started IP: %s", WiFi.softAPIP().toString().c_str());
      net.openingSoftAP = false;
      net.softAPOpened = true;
      break;
    case ARDUINO_EVENT_WIFI_AP_STOP:
      if(!net.openingSoftAP) LOGD("(evt) WiFi SoftAP Stopped");
      net.softAPOpened = false;
      break;      
    default:
      if(event > ARDUINO_EVENT_ETH_START)
        LOGD("(evt) Unknown Ethernet Event %d\n", event);
      break;
  }
}
