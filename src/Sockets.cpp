  ...
  for(uint8_t i = 0; i < sizeof(this->newClients); i++) {
    ...
    if(slot != 255) {
      ...
      if(asyncId != 0 && ws.hasClient(asyncId)) {
        LOGI("Initializing Socket Client %u (asyncId=%lu)", slot, asyncId);
        esp_task_wdt_reset();
    ...
    case WS_EVT_CONNECT:
      {
        ...
        if(slot == 255) {
          LOGE("Socket: No free client slots, closing %lu", asyncId);
          client->close();
          return;
        }
        IPAddress ip = client->remoteIP();
        LOGI("Socket [%lu] Connected from %d.%d.%d.%d (slot %u)", asyncId, ip[0], ip[1], ip[2], ip[3], slot);
    ...
    case WS_EVT_DISCONNECT:
      {
        ...
        LOGD("Socket [%lu] Disconnected (slot %u)", asyncId, slot);
        if(slot != 255) {
          for(uint8_t i = 0; i < SOCK_MAX_ROOMS; i++)
    ...
    case WS_EVT_DATA:
      {
        ...
        if(info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
          ...
          if(strncmp((char*)data, "join:", 5) == 0) {
            ...
            LOGD("Client %u joining room %u", slot, roomNum);
            ...
            LOGD("Client %u leaving room %u", slot, roomNum);
            if(roomNum < SOCK_MAX_ROOMS && slot != 255) sockEmit.rooms[roomNum].leave(slot);
          }
          else {
            LOGD("Socket [%lu] text: %s", asyncId, data);
          }
        }
      }
      break;
    case WS_EVT_ERROR:
      LOGE("Socket [%lu] Error", asyncId);
      break;
    case WS_EVT_PONG:
      break;
  }
